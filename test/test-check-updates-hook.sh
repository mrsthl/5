...
# Test 8: Statusline update indicator
echo "Test 8: Statusline update indicator"
echo "------------------------------------"
STATUSLINE_SCRIPT="$PROJECT_ROOT/src/hooks/statusline.js"
TEST_DIR="/tmp/test-check-updates-8"
rm -rf "$TEST_DIR"
mkdir -p "$TEST_DIR/.5"
cd "$TEST_DIR"
