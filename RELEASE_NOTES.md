...
> **Prerelease.** Published to the npm `beta` dist-tag. Install with `npx foifi@beta`. `npx foifi` and `/5:update` continue to serve the stable 2.1.0 line.

### Wrap Native Capabilities Instead of Reimplementing Them

foifi was built when Claude Code had no plan mode, no task tracking, and no review skill, so it carried hand-rolled equivalents. Those capabilities now exist and are better than the substitutes. v3.0 delegates to them and keeps only what foifi genuinely adds: the durable repo-local artifact, the ticket and config conventions, and the delegation strategy for large plans.

Net effect across the two commits in this beta: **-628 lines** of workflow prose and hook code.
...
**Improvements**
- **Deterministic resume** (`src/workflows/5-implement.js`, `src/agents/step-orchestrator-agent.md`): component names are now pinned verbatim to the plan's Component Checklist. The execution graph used to be persisted precisely because re-deriving it could rename components and break resume matching; fixing the cause lets resume match on `completedComponents` alone.
- **Prerelease-aware publishing** (`.github/workflows/publish.yml`): `npm publish` defaults to the `latest` dist-tag and does not special-case semver prereleases. Betas now derive `--tag beta`, mark the GitHub Release as a prerelease, and no longer push a version bump to `main`.
- **Leaner helpers**: `/5:split` 190 → 60 lines and `/5:commit` 112 → 40 lines, keeping the naming conventions and config-driven message logic.
- **Contract tests** (`test/verify-install-js.sh`): a Plan Mode Contract block with 11 assertions guards the wrap, the fallback, the refine mode, and the state shape.
...
v2.1.0 modernizes `/5:implement` on Claude Code to run through the native Workflow tool when available, executing components in parallel waves with inline verification. The prose loop in `implement.md` remains the fallback for Claude Code and the only path for Codex, preserving full runtime parity.

**What's New**
- **Workflow orchestration** (`src/workflows/5-implement.js`): a new Workflow-tool script drives the implement loop — the orchestrator, executor, and verifier prompts are kept in sync with `src/agents/*-agent.md`. Components run in parallel waves and are verified inline.
...
### Split Configure Skills and Rebuildable Codebase Index

Splits the configuration generation logic into dedicated skills and adds a rebuildable codebase index flow that configure/reconfigure can generate and refresh for any project.

**What's New**
- **Rebuildable codebase index**: Configure now plans a generated `.5/index/` workspace containing a repo-local `rebuild-index.sh`, multiple focused Markdown index files, and a manifest file for quick AI/project context loading
- **Index freshness guidance**: Generated `CLAUDE.md` instructions now require the index to be regenerated by rerunning the script when the index is older than one day
- **Dedicated docs/index skill**: Added `configure-docs-index` to own `.5/*.md` documentation, `.5/index/*`, and `CLAUDE.md`

**Improvements**
- **Split configure skill responsibilities**: The old all-in-one configure skill is now split so `configure-skills` owns `create-*`, `run-*`, and workflow-generated rules, while `configure-docs-index` owns documentation and indexing
- **Leaner configure prompts**: `configure.md` and `reconfigure.md` now keep the command-level contract concise and delegate detailed index-generation behavior to the dedicated skill
- **Explicit requirement-to-skill mapping**: The generated CONFIGURE feature spec now states which skill handles documentation/indexing versus skills/rules generation, reducing ambiguity during planning and implementation

**Affected files:**
- `src/commands/5/configure.md` (modified)
- `src/commands/5/reconfigure.md` (modified)
- `src/skills/configure-docs-index/SKILL.md` (new)
- `src/skills/configure-skills/SKILL.md` (modified/renamed from `configure-project`)
- `README.md` (modified)
- `CLAUDE.md` (modified)
